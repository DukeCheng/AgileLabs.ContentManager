...
using ControllerBase = AgileLabs.ContentManager.WebFramework.ControllerBase;
...
namespace AgileLabs.ContentManager.Areas.Admin.Controllers
{
    ...
    {
        private readonly MongoDbBaseRepository<Resource> _resourceRepository;
        ...
        public IActionResult ManageResource(Guid resourceId)
        {
            return View();
        }
        ...
        [HttpGet]
        public IActionResult GetReourcesList(int page = 1, int rows = 10)
        {
            var pagedRecords = _resourceRepository.PaginationSearchAsync(Builders<Resource>.Filter.Empty,
                Builders<Resource>.Sort.Descending(x => x.CreationTime), page, rows).Result;

            return Ok(new EasyUIPage<Resource>
            {
        ...
        public IActionResult Remove(Resource resource)
        {
            var result = _resourceRepository.Delete(resource);

            if (result)
                return Ok(new WebResponseModel
                {
                    methodName = nameof(Remove)
                });
            return Ok(new WebResponseModel()
            {
                error = true,
